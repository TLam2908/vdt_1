     ...
     {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-white">
          <div className="flex justify-between items-center p-4 border-b">
            ...
            <button className="text-gray-500 hover:text-black" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu">
              <X className="h-6 w-6" />
            </button>
          </div>
          <nav className="p-4">
            <ul className="space-y-4">
              {navLists.map((item, index) => (
                <li key={index} className="border-b border-gray-100 pb-4">
                  <div className="text-lg font-medium" onClick={() => setMobileMenuOpen(false)}>
                    {item}
                  </div>
                </li>
              ))}
